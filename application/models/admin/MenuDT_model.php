 ...
 */
class MenuDT_model extends CI_Model {

	var $table = 'menu';
	var $column_order = array(null,'p.menu_id','p.name'); //set column field database for datatable orderable
	var $column_search = array('p.menu_id','p.name','p.url'); //set column field database for datatable searchable 
	var $order = array('p.menu_id' => 'desc'); // default order 
	...
	private function _get_datatables_query()
	{
		$store_id = $this->session->userdata('admin_data')['store_id'];
                $this->db->select("p.menu_id,p.name,p.url,p.status,p.menuorder");
		$this->db->from("menu p");
