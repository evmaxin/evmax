...
class Banners extends CI_Controller {
	...
	{
           // $this->load->model('admin/Banners_model','alias');
           $this->load->model('admin/Category_model','category');
            $data['title']='Banners';
            //$data['totaldata'] = $this->attributetype->get_attributetype_array();
            $data['datatable_path']= "admin/Banners/ajax_list";// must and should declare this value
   ...
   private function set_upload_options()
    {   
        ...
        $config = array();
        $config['upload_path'] = './public/uploads/admin/banners';
	...
	public function add() {
       ...
       //Sliders adding with multiple images
            ...
            if ($cpt>0) {
                for ($i = 0; $i < $cpt; $i++) {
                 ...
                 {
                    $this->db->insert_batch('banners', $image_parameters_batch);
                 }
        ...
        public function update($id) {
            ...
            if(isset($id) && ($id>0) && isset($_POST['update'])){
             ...
             if (file_exists("public/uploads/admin/banners/$row->name")) {
                 unlink( "public/uploads/admin/banners/$row->name" );
       ...
       //Sliders adding with multiple images
        ...
        public function edit($id) {
            ...
            $template['content'] = $this->load->view('admin/banners/edit', $data, TRUE);
            $this->load->view('admin/template',$template);//For data tables only template
            
        }
  	 public function ajax_list()
	{
             $this->load->model('admin/BannersDT_model','alias');
		$list = $this->alias->get_datatables();
		
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $customers) {
                    $full_url = base_url()."admin/banners/edit/".$customers->slider_id;
                    $image_url = "<img style='width: 20%;' src=".base_url()."public/uploads/admin/banners/".$customers->name.">";
			$no++;
			$row = array();
			$row[] = $no;
			$row[] = $image_url;
