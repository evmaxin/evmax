...
class ChargingStation extends CI_Controller {

	/**
         * @file        : ChargingStation
         * @type        : Controller
         * @author      : Nagender
	...
	public function index() {
        ...
        $data['datatable_path'] = "admin/ChargingStation/ajax_list"; // must and should declare this value
        ...
        $template['content'] = $this->load->view('admin/chargingstation/index', $data, TRUE);
	...
	public function add() {
         ...
         $this->load->model(array('Common_model' => 'common'));
          ...
          $res = $this->common->insertdata($tablename = "markers", $parameters);
            

             //image upload
    
             if ($res>0) {
            $this->session->set_flashdata('success', 'Charging Station details added successfully');
             }
           redirect('/admin/ChargingStation/');
        }
	...
	{   
             ...
             $this->input->post('target_tab');
                $models = array(
                            ...
                            'admin/ChargingStation_model' =>'product',
		...
		if(!isset($_POST['update']))
		{
         ...
         //   $data['attributes'] = $this->attribute->get_attributes();
            ...
            $template['content'] = $this->load->view('admin/chargingstation/edit', $data, TRUE);
...
//$img = preg_replace('/\d/', '',$_FILES['userfile']['name']);
 //$image_path = preg_replace('/\d/', '', $img );
	...
	{
             $this->load->model('admin/ChargingStationDT_model','alias');
		$list = $this->alias->get_datatables();
		//echo $this->db->last_query(); exit;
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $product) {
                        $full_url = base_url()."admin/ChargingStation/edit/".$product->id;
                        $view_url = base_url()."admin/ChargingStation/details/".$product->id;
			$no++;
			$row = array();
                       $row[] = $product->name;
                       $row[] = $product->lat;
