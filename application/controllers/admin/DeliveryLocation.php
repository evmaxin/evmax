...
class DeliveryLocation extends CI_Controller {
	...
	public function index() {
        ...
        $this->load->model(array('admin/DeliveryLocation_model' => 'category','Common_model' => 'common'));
        $data['datatable_path'] = "admin/DeliveryLocation/ajax_list"; // must and should declare this value
        $data['states'] = $this->common->getTableData($tablename = "state", $cols = "state_id,state_name,status",array(),"asc","state_name");
        $template['content'] = $this->load->view('admin/deliverylocation/index', $data, TRUE);
	...
	public function add() {
         ...
         $this->load->model(array('Common_model' => 'common'));
          ...
          $res = $this->common->insertdata($tablename = "delivery_location", $parameters);
          ...
          //print_r($parameters); exit;
           ...
           redirect('/admin/DeliveryLocation/');
        }
	...
	{   
             ...
             $this->input->post('target_tab');
                $models = array(
                            'admin/DeliveryLocation_model' =>'alias',
                           'Common_model' => 'common'
                                );
		...
		if(!isset($_POST['update']))
		{
            ...
            $template['content'] = $this->load->view('admin/deliverylocation/edit', $data, TRUE);
	...
	//$data['attributes'] = $this->attribute->get_attributes();
         ...
         $parameters = array(
             ...
             $result = $this->common->update($table='delivery_location', $parameters, $where_cond);

            $target_tab =$this->input->post('target_tab')?$this->input->post('target_tab'):'1';
           // $this->session->set_userdata('target_tab',$target_tab);
        if($result) { redirect('/admin/DeliveryLocation/index/'); }
    }
  
    public function ajax_list()
	{
             $this->load->model('admin/DeliveryLocationDT_model','alias');
		$list = $this->alias->get_datatables();
		//echo $this->db->last_query(); exit;
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $product) {
                        $full_url = base_url()."admin/DeliveryLocation/edit/".$product->id;
                        $view_url = base_url()."admin/DeliveryLocation/details/".$product->id;
			$no++;
			$row = array();
